...
%
% ⚙ LAYOUT = BY-CONSTRUCTION (positioning + fit + node anchors): the 6
%   zones are chained with right=of (one zone gap), inter-zone arrows ride
%   node anchors, each sub-panel's internals sit in a scope
%   shift={(zone.center)} so they AUTO-RECENTER. Change content / move a
%   zone and the rest REFLOWS — no coordinate bookkeeping.
%   ⚠ The "SUB-PANEL CENTERING" x-shift numbers (15.45 / 20.7) below are
%   the ORIGINAL absolute-coord values — they NO LONGER EXIST in the body
%   (sub-panels self-center via scope shift); ignore "recompute shifts".
%   SEMANTIC constraints (renaming, MDT U-shape feedback) STILL HOLD.
%
...
% ---- single source of truth for the zone-row rhythm ----
% Each macro zone is an invisible fixed-size node; content is shifted to
% its .center. Zone heights all equal \zoneh so the band lines up.
\newlength{\zoneh}\setlength{\zoneh}{11cm}   % zone band height (y 4.5..15.5)
\def\zonesep{0.55cm}                          % the ONE inter-zone gap
...
\begin{tikzpicture}[
    ...
    % invisible macro-zone anchor node: fixed footprint, content shifts to .center
    zone/.style={rectangle, minimum height=\zoneh, anchor=north west,
...
% ================================================================
%  MACRO ZONE FRAME — 6 invisible anchor nodes, chained left to right.
%  Widths chosen to reproduce original footprints; only the FIRST is
%  hand-placed, the rest follow via right=of + the single \zonesep gap.
% ================================================================
\node[zone, minimum width=4.0cm] (zInput)                       at (-0.5, 15.5) {};
\node[zone, minimum width=3.0cm, right=\zonesep of zInput] (zProj)    {};
\node[zone, minimum width=7.4cm, right=\zonesep of zProj]  (zCMT)     {};  % HERO
\node[zone, minimum width=4.0cm, right=\zonesep of zCMT]   (zDiag)    {};
\node[zone, minimum width=4.0cm, right=\zonesep of zDiag]  (zExplain) {};
\node[zone, minimum width=3.4cm, right=\zonesep of zExplain] (zValid) {};

% ================================================================
%  STAGE 1: MULTIMODAL FEATURE EXTRACTION  (inside zInput)
% ================================================================
\begin{scope}[shift={(zInput.center)}]
  % --- CT pathway (top) ---
  \node[blue_node, minimum width=3.0cm, minimum height=2.0cm] (ct) at (0, 4)
  ...
  \node[green_node, minimum width=3.0cm, minimum height=2.0cm] (path) at (0, 0)
      {\textbf{病理切片}\\[1pt]{\footnotesize ViT-B/16}\\[-1pt]{\scriptsize 768维}};
  % --- EHR pathway (bottom) ---
  \node[orange_node, minimum width=3.0cm, minimum height=2.0cm] (ehr) at (0, -4)
      {\textbf{电子病历}\\[1pt]{\footnotesize 临床 BERT}\\[-1pt]{\scriptsize 768维}};
\end{scope}

% --- Projection layers (inside zProj, vertically aligned to inputs) ---
\node[proj_box] (projCT)   at (zProj.center |- ct)   {\scriptsize 投影\\[-1pt]\scriptsize 512维};
\node[proj_box] (projPath) at (zProj.center |- path) {\scriptsize 投影\\[-1pt]\scriptsize 512维};
\node[proj_box] (projEHR)  at (zProj.center |- ehr)  {\scriptsize 投影\\[-1pt]\scriptsize 512维};

% ================================================================
%  STAGE 2: CROSS-MODAL TRANSFORMER FUSION  (inside zCMT)
% ================================================================
% CMT large box (core module). Centered horizontally in zCMT, centered
% vertically on the middle (path) input row.
\node[red_node, minimum width=6.5cm, minimum height=9.5cm, line width=1.5pt]
      (cmt) at (zCMT.center |- path) {};
\begin{scope}[shift={(cmt.center)}]
  \node[font=\small\bfseries, text=drawRedLine!80!black] at (0, 4.0)
      {跨模态 Transformer (CMT)};
  \node[font=\footnotesize, text=drawRedLine!60!black] at (0, 3.4)
      {6层交叉注意力};
  % Inside CMT: Cross-attention diagram (simplified) — left column
  \node[small_box, fill=drawBlueFill!50, draw=drawBlueLine!70,
        minimum width=1.8cm] (qry) at (-1.7, 2.0) {\scriptsize Query (CT)};
  \node[small_box, fill=drawGreenFill!50, draw=drawGreenLine!70,
        minimum width=1.8cm] (key) at (-1.7, 1.0) {\scriptsize Key (病理)};
  \node[small_box, fill=drawOrangeFill!50, draw=drawOrangeLine!70,
        minimum width=1.8cm] (val) at (-1.7, 0.0) {\scriptsize Value (EHR)};
  % Attention heatmap (6x6) — right of the Q/K/V column
  ...
  \end{scope}
  % MCG module inside CMT (bottom)
  \node[purple_node, minimum width=2.2cm, minimum height=2.2cm,
        inner sep=6pt] (mcg) at (0, -3.0)
      {\textbf{MCG}\\[1pt]{\scriptsize 模态门控}\\[-1pt]{\scriptsize CT\,0.45}\\[-1pt]{\scriptsize 病理\,0.35}\\[-1pt]{\scriptsize EHR\,0.20}};
\end{scope}

% ================================================================
%  STAGE 3: HIERARCHICAL DIAGNOSIS  (inside zDiag)
% ================================================================
% Absolute row heights reused from the original; combined with zone x via -|
\coordinate (yCoarse)  at (0, 13.0);
\coordinate (yFine)    at (0,  8.5);
\coordinate (yGradcam) at (0, 13.5);
\coordinate (yShap)    at (0,  9.5);
\coordinate (yGpt4)    at (0,  5.8);
% Coarse classification (top)
\node[green_node, minimum width=3.2cm, minimum height=1.8cm] (coarse)
    at (yCoarse -| zDiag.center) {\textbf{粗分类头}\\[1pt]{\footnotesize 5类疾病大类}\\[-1pt]{\scriptsize 准确率 96.2\%}};
% Fine classification (larger box with embedded bar chart)
\node[green_node, minimum width=3.6cm, minimum height=4.5cm] (fine)
    at (yFine -| zDiag.center) {};
\begin{scope}[shift={(fine.center)}]
  \node[font=\small\bfseries, text=drawGreenLine!80!black] at (0, 1.8) {细分类头};
  \node[font=\footnotesize, text=drawGreenLine!60!black] at (0, 1.2) {23个具体病种};
  % Embedded accuracy comparison bar chart (group center on fine.center.x)
  % bars 粗(-0.75..-0.15)+细(0.15..0.75) -> group center 0; chart origin
  % offset so the group is horizontally centered in the box.
  \begin{scope}[shift={(-0.75, -1.7)}]
...
\node[tag, text=drawOrangeLine, fill=white, fill opacity=0.9, text opacity=1,
      anchor=west] at ($(coarse.south east)+(0.0,-0.4)$) {\scriptsize 层次化Softmax};

% ================================================================
%  STAGE 4: EXPLAINABILITY  (inside zExplain)
% ================================================================
% Grad-CAM with mini heatmap (top)
\node[blue_node, minimum width=3.2cm, minimum height=2.5cm] (gradcam)
    at (yGradcam -| zExplain.center) {};
\begin{scope}[shift={(gradcam.center)}]
  \node[font=\footnotesize\bfseries, text=drawBlueLine!80!black] at (0, 0.8) {Grad-CAM};
  % Mini heatmap (4x4 grid); 4 cells x 0.4 = 1.6 wide -> center the grid:
  % origin offset (-0.8, 0.1) so the heatmap is centered under the title.
...
% SHAP analysis with horizontal bar chart (middle)
\node[orange_node, minimum width=3.2cm, minimum height=3.5cm] (shap)
    at (yShap -| zExplain.center) {};
\begin{scope}[shift={(shap.center)}]
  \node[font=\footnotesize\bfseries, text=drawOrangeLine!80!black] at (0, 1.4) {SHAP 分析};
  % Embedded horizontal bar chart (local origin near bottom-left of box)
  \begin{scope}[shift={(-1.3, -1.5)}]
...
% GPT-4 diagnosis report (bottom)
\node[purple_node, minimum width=3.2cm, minimum height=1.8cm] (gpt4)
    at (yGpt4 -| zExplain.center)
    {\textbf{GPT-4}\\[1pt]{\footnotesize 诊断报告生成}};

% ================================================================
%  STAGE 5: CLINICAL VALIDATION  (inside zValid)
% ================================================================
\node[red_node, minimum width=2.8cm, minimum height=2.5cm, inner sep=6pt]
    (mdt) at (zValid.center |- path)
    {\textbf{临床验证}\\[2pt]{\scriptsize 双盲审}\\[-1pt]{\scriptsize Kappa=0.87}\\[2pt]{\scriptsize MDT 讨论}};

% ================================================================
%  ZONE BACKGROUNDS — drawn from each zone's own anchors (no re-typed grid)
% ================================================================
\begin{pgfonlayer}{background}
  \foreach \z/\fillc/\linec in {%
      zInput/drawBlueFill!10/drawBlueLine!30,
      zProj/drawGreyFill!30/drawGreyLine!30,
      zCMT/drawRedFill!8/drawRedLine!25,
      zDiag/drawGreenFill!10/drawGreenLine!30,
      zExplain/drawOrangeFill!10/drawOrangeLine!30,
      zValid/drawRedFill!10/drawRedLine!25} {
    \fill[\fillc, rounded corners=8pt] (\z.south west) rectangle (\z.north east);
    \draw[\linec, dashed, thick, rounded corners=8pt] (\z.south west) rectangle (\z.north east);
  }
\end{pgfonlayer}

% ================================================================
%  AUC COMPARISON PANEL (bottom) — fit() around its drawn content
% ================================================================
% The bar chart and legend are drawn first (in local coords anchored to a
% reference coordinate), then a fit() box wraps them as the panel border.
\coordinate (aucOrigin) at ($(zInput.south west)+(0.5,-4.25)$);
\begin{scope}[shift={(aucOrigin)}]
  % AUC bar chart
  ...
  \end{scope}
  % invisible corner markers so fit() captures the full panel extent
  \node (aucBL) at (0.0, -0.5) {};
  \node (aucTR) at (21.8, 3.0) {};
\end{scope}
% Panel border via fit() around the drawn content
\begin{pgfonlayer}{background}
  \node[rectangle, rounded corners=6pt, draw=drawBlueLine, fill=drawBlueFill!12,
        thick, inner sep=0pt, fit=(aucBL)(aucTR)] (aucpanel) {};
\end{pgfonlayer}
% Panel title — anchored above the panel's own north west corner
\node[font=\small\bfseries, text=drawBlueLine!80!black, anchor=south west]
    at ($(aucpanel.north west)+(0.2,0.1)$) {多模态融合性能对比 (AUC)};

% ================================================================
%  CONNECTIONS — Core data flow (all via NODE ANCHORS)
% ================================================================
...
% CMT -> Classification
\draw[data_arrow, rounded corners=6pt] (cmt.east |- coarse) -- (coarse.west);
\draw[data_arrow, rounded corners=6pt] (cmt.east |- fine) -- (fine.west);

% Classification -> Explainability
\draw[arrow, rounded corners=6pt] (coarse.east) -- ++(0.8, 0) |- (gradcam.west);
\draw[arrow] (fine.east) -- (shap.west);
\draw[arrow, rounded corners=6pt] (fine.east) -- ++(0.8, 0) |- (gpt4.west);

% Explainability -> Clinical validation
\draw[arrow, rounded corners=6pt] (gradcam.east) -- ++(0.5, 0) |- (mdt.west |- gradcam);
\draw[arrow] (shap.east) -- (mdt.west);
\draw[arrow, rounded corners=6pt] (gpt4.east) -- ++(0.5, 0) |- (mdt.west |- gpt4);

% ================================================================
%  MDT FEEDBACK LOOP (red dashed, orthogonal U-shape along the bottom)
% ================================================================
% DOWN past the AUC panel bottom, LEFT to the far margin, UP to the
% input row, then RIGHT into Pathology. All turns reference node anchors
% / panel anchor so the U-shape tracks the layout under reflow.
\coordinate (loopBot) at ($(aucpanel.south)+(0,-0.75)$);
\draw[feed_arrow, rounded corners=8pt, line width=1.2pt]
    (mdt.south) |- (mdt.south -| loopBot) -- (loopBot)
    -- (zInput.west |- loopBot) -- (zInput.west |- path) -- (path.west);
% Label in the empty space along the bottom horizontal segment
\node[font=\footnotesize\bfseries, text=drawRedLine,
      fill=white, fill opacity=0.9, text opacity=1,
      inner sep=3pt, rounded corners=2pt] at (loopBot) {MDT 反馈优化};

% ================================================================
%  STAGE LABELS (top) — anchored to each zone .north, follow the zone
% ================================================================
\node[font=\footnotesize\bfseries, text=drawBlueLine!80!black,
      fill=drawBlueFill!60, draw=drawBlueLine!40, rounded corners=3pt,
      inner sep=4pt, anchor=south] at ($(zInput.north)+(0,0.3)$) {阶段一：多模态特征提取};
\node[font=\footnotesize\bfseries, text=drawGreyLine!80!black,
      fill=drawGreyFill!80, draw=drawGreyLine!40, rounded corners=3pt,
      inner sep=4pt, anchor=south] at ($(zProj.north)+(0,0.3)$) {投影};
\node[font=\footnotesize\bfseries, text=drawRedLine!80!black,
      fill=drawRedFill!60, draw=drawRedLine!40, rounded corners=3pt,
      inner sep=4pt, anchor=south] at ($(zCMT.north)+(0,0.3)$) {阶段二：跨模态注意力融合};
\node[font=\footnotesize\bfseries, text=drawGreenLine!80!black,
      ...
      inner sep=4pt, anchor=south] at ($(zDiag.north)+(0,0.3)$) {阶段三：层次化诊断};
\node[font=\footnotesize\bfseries, text=drawOrangeLine!80!black,
      ...
      inner sep=4pt, anchor=south] at ($(zExplain.north)+(0,0.3)$) {阶段四：可解释性};
\node[font=\footnotesize\bfseries, text=drawRedLine!80!black,
      fill=drawRedFill!60, draw=drawRedLine!40, rounded corners=3pt,
      inner sep=4pt, anchor=south] at ($(zValid.north)+(0,0.3)$) {阶段五};

% ================================================================
%  DATA VOLUME ANNOTATION (centered under 阶段五 label, inside zValid)
% ================================================================
\node[font=\scriptsize, text=drawGreyLine, fill=drawGreyFill!50,
      draw=drawGreyLine!40, rounded corners=3pt, inner sep=4pt,
      anchor=north] at ($(zValid.north)-(0,0.2)$)
