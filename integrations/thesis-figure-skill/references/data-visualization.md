...
数据可视化混合图信息密度高，容易把字缩得太小；但**只给下限会变成单向棘轮**——
"放大字号"在规则体系里成了纯收益、零成本的动作，最终把版面挤爆。下限与上限必须成对给。

| 元素 | 下限（不得更小） | 上限（不得更大） |
|------|---------|---------|
| 框标题/模块名 | `\small\bfseries` | 比整图主标题小 **≥2 档** |
| 坐标轴标签 | `\footnotesize` | 不得大于同图框内正文 |
| 数据标注（百分比、数值） | `\footnotesize` | 不得大于所标注对象的标题 |
| 公式 | `\footnotesize` | 不得大于框标题 |
| 图例文字 | `\footnotesize` | 不得大于框内正文 |

**绝对禁止 `\tiny`**——在 300dpi PNG 中 `\tiny` 文字几乎不可读。

**空间不够时怎么办——取决于画布是否已定：**

| 情形 | 正确做法 | 禁止 |
|---|---|---|
| **画布尺寸未定**（设计早期） | 加大框尺寸，画布随之变大 | 缩小字号到下限以下 |
| **画布尺寸已定**（多数情况） | ① 缩短文案 ② 改两行 ③ 重排该区布局 | **放大字号或框尺寸**——画布不动而元素变大＝挤压全图 |

> ⚠️ 实测教训：把「加大框尺寸，不要缩小字」当成无条件指令，在画布已定的图上
> 导致正文 +23%、区标题 +38% 而画布纹丝不动，每个框单看都更清楚、合起来全部顶满边框。
> 详见 `lessons.md`「字号单向棘轮」条。
