...
3. 固定五个核心经营维度的唯一主落点：`5.1 企业战略`、`5.4 运营管理`、`5.5 财务状况`、`6.3 市场竞争`、`7.1 商业模式`。相关内容可以跨章回引，但不得重复套框架或在主落点缺少实质判断；没有公开财务时使用可核验代理指标并披露局限，绝不虚构报表或数值。
4. 使用 [report-framework-approval-template.md](assets/report-framework-approval-template.md) 输出到三级标题。每个三级标题下写 2—3 行结合本项目材料的内容概括，至少包含“内容概括”和“证据与边界”；缺证时保留 `[待核验]`、`[待调研]` 或 `[待验证假设]`，不得用通用经验补成企业事实。
5. 发展瓶颈最终保留 2—4 项。可选结果导向结构（每个二级标题直接概括“对象/错配/约束 + 后果”）或分类导向结构（二级为内部/外部/地区等挑战，三级为结果导向瓶颈）；两种结构都必须逐项写症状与差距、直接原因与结构根因、经营影响、反证和边界。
