...
description: 辅导全国高校商业精英挑战赛创新创业竞赛流通业经营模拟赛道的完整备赛流程：项目简报、企业选题与同校查重、地区调研与证据台账、国一经验蒸馏后的分析报告框架和三批分章节写作、权威来源与数据图表验证、中文Word正文去AI味与最小改写、专业问卷/访谈附录、中文研究报告Word版式、可编辑图形母版与学术图解润色、自动图题表题与逐页视觉门禁、官方60分证据评分复核、恰好20页PPT初版大纲及frontend-slides制作、证据绑定的10分钟逐页路演稿与真人彩排，以及可选5分钟答辩和最终合规检查。用于用户提到流通业经营模拟赛道、企业经营案例分析报告、流通业计划书、批发/零售/餐饮/物流/供应链/冷链/即时零售案例、选题查重、调研、地区性分析、报告正文、Word排版、去AI味、可编辑配图、Canva母版、图表题注、数据可视化、报告评分、20页PPT、10分钟陈述或答辩时；支持研究生、本科和高职组。必须区分本赛道与创业计划赛道，不得虚构证据或代写成可冒充完全由人类原创的终稿。
---
...
| 3 报告 | 用户确认框架、三批分章节 Word、可编辑图形母版登记、合并 Word/PDF | 确认单采用国一共性蒸馏的固定 11 章三级框架；B01含研究框架图、年报数字画像和两种通过数据门槛的实证方法；B01/B02/B03各自形成纯读者正文DOCX并通过去AI味、语言泄漏、可编辑母版、逐页图文和版式复核后交用户确认；五个核心经营维度、2—4 项瓶颈及全文一致性通过 |
...
- **报告正文与 Word**：框架确认后必须读 [section-writing-evidence-protocol.md](references/section-writing-evidence-protocol.md)、[reader-facing-report-style.md](references/reader-facing-report-style.md)、[human-chinese-word-style.md](references/human-chinese-word-style.md)、[annual-report-empirical-methods.md](references/annual-report-empirical-methods.md)、[word-visual-production.md](references/word-visual-production.md) 和 [excalidraw-imagegen-integration.md](references/excalidraw-imagegen-integration.md)；写B02/B03还必须读 [national-first-b02-b03-writing-visual-patterns.md](references/national-first-b02-b03-writing-visual-patterns.md)。内部施工复制 [report-section-batch-template.md](assets/report-section-batch-template.md)，去AI味复核复制 [humanization-review-template.md](assets/humanization-review-template.md)，两者均不得导出；读者正文按批次复制 `report-reader-b01/b02/b03-template.md`。B02/B03先复制 [report-visual-map-template.json](assets/report-visual-map-template.json) 并运行 `scripts/validate_report_visual_map.py`。数据先调用 `$data-analytics:validate-data`，程序分析调用 `$data-analytics:jupyter-notebooks`，统计图调用 `$data-analytics:visualize-data`，每批DOCX调用 `$documents:documents`。每一批正文配图都必须调用并记录外部 `$excalidraw-diagram`、`$thesis-figure-skill` 和系统 `$imagegen` 的路由结果：Excalidraw负责研究框架、价值链、流程、机制、泳道和路线图，thesis-figure-skill负责学术图形的构图审查以及适配的公式/TikZ/高密度机制图重绘，imagegen只辅助适配的无文字位图或视觉素材。某工具不适合实际出图时也要记录 `INVOKED_NOT_APPLICABLE` 及理由，不能为了“调用齐全”生成装饰图。三者均不得替代数据图、精确标签或真实证据。所有结构图须保留可编辑源文件并完成渲染复核；图内不得含题注，图内所有可见文字在最终Word宽度下不得小于14pt，关键节点、主标签与关键数字采用16pt。信息放不下时拆图或移到正文，不得缩字。用户要求Canva可编辑图时还必须读 [canva-editable-figure-workflow.md](references/canva-editable-figure-workflow.md)，复制 [editable-figure-register-template.json](assets/editable-figure-register-template.json) 并运行 `scripts/validate_editable_figure_register.py`。`$canva:canva-edit-design` 只编辑已有Canva设计；没有设计链接或ID时记录 `AWAITING_DESIGN_ID`，不得声称已转换。Canva母版可编辑不等于Word对象可编辑；真正需要Word内编辑时优先使用Word原生表格、图表和形状。Kami 1.12.0 只作为版式与视觉语言参考；这些工具都不得写进参赛正文，也不能冒充统计证据或Word生成器。
...
2. 逐家核实企业主体、流通业属性、典型性、本省实际经营和可获得的一手/公开证据。若后续要求采用年报和实证模型，还须把连续年报/正式经营数据及外部同口径数据的可得性作为硬门槛；没有年报或经核验等价资料时提前提示补数或换题。全国知名、可网购或偶发宣传不能单独证明本省布局。
...
建立证据台账并保存标题、机构、日期、URL/文件、页码、地区、期间、指标口径、主张ID、限制与人工核验状态。问卷须包含知情说明、筛选/跳题、最近一次行为、构念映射、预测试、清洗和变量编码；访谈按管理层、门店/网点负责人、一线员工、供应商/加盟商、消费者等角色分开，并保留参与/录音/材料同意、匿名编号、反证和主题代码本。完成预测试后再正式执行访谈、问卷、门店/网点观察和同商圈竞品对标。模拟回答不得作为真人数据。
...
4. 使用 [report-framework-approval-template.md](assets/report-framework-approval-template.md) 输出到三级标题。概要固定两个二级标题：`2.1 研究对象与核心问题`、`2.2 主要结论与方案方向`，各用一个三级标题凝练展开。引言使用正式书面标题 `3.1 研究背景与选题价值`、`3.2 研究设计与范围`；引言必须规划一张研究框架图。案例简介保持文字凝练，但用企业年报的可追溯数字、趋势/结构图和两种合格模型的结果概览提高证据密度。每个三级标题下写2—3行结合项目材料的内容概括和证据边界，缺证时保留待处理状态，不得用通用经验补成企业事实。
...
6. 第11章“附录及参考资料”固定六个二级标题：企业重要数据概览、调查问卷、访谈纲要（含知情说明与编码）、参考文献、观察/竞品与现场记录、补充图表与地区对标。不得用“调研工具”“补充材料”等合并标题替代或遗漏任一项。问卷和访谈须使用本Skill专业模板并与真实执行版本一致；参考文献采用现行GB/T 7714—2025顺序编码制的期刊式普通段落列表，悬挂缩进，禁止表格排版。
7. 框架中的企业事实、数字、因果、瓶颈和方案依据必须登记为 `[已核验:E###]` 并给出同一证据 ID 的精确定位；证据不足时只写允许的待处理状态。**只输出框架并等待用户确认**，不得提前写长篇正文。
8. 交付确认单前运行：
...
12. 第4—7章开写前建立“章节主张—证据包”。正文必须综合专业文献、政府/统计/监管等公开权威机构报告或数据、企业官方披露/交易所文件、权威网站，并按章节需要加入同地区同场景竞品分析、消费者洞察、产品分析和财务分析。第8—10章的差距、根因、经营影响、方案参数和结论同样必须回指第4—7章已核验证据或新增的可定位证据；不得为凑齐类别引用无关材料，不可访问、付费墙内或只有摘要的资料不得被写成已阅读全文。
13. 内部施工文件按“结论 → 证据 → 对标 → 机制 → 决策含义 → 边界”绑定证据ID和精确定位；读者正文则采用结论先行、数据支撑、机制解释、经营含义收束的专业咨询/企业经营分析口吻，并用人类可读脚注、图下注或参考文献呈现来源。完成初稿后执行最小改写：先锁定数字、术语、来源、否定、限制和因果强度，再只修复口语、空泛渲染、机械连接、欧化长定语、被动句堆积和逻辑跳跃；原文已经自然准确时保持不变。内部的 `[待团队核实]`、`E###`、修改日志和状态码不得进入 Word；存在P0缺口时不把该批标为完成稿。
14. 只使用能回答明确问题的方法，不堆砌 PESTEL、SWOT、五力、画布或实证模型；方法输出不是证据。竞品、消费者、产品与财务分析分别满足同口径、样本边界、产品对象和财务期间/单位一致性要求。
15. 每项方案明确根因、目标对象、流程动作、责任主体、0–3个月试点、3–12个月推广、资源/预算、KPI、验证、风险、触发器和回滚。方案的提升幅度、节省、增量收入、投资回报、回收期和推广覆盖均只能标为目标范围、情景测算或待试点验证；未取得真实试点或合格对照数据前不得写成已实现结果。
16. B02/B03在不改变已确认三级框架的前提下，执行国一样本蒸馏规范：每个小节从对比、流程、场景或诊断中选择适配的自然叙述路径，不机械重复同一连接词；一段保留1—3个锚点数据，其余精确值进入数值表。开写前制作图表地图，逐项审查数据表、折线、热力、词云、柱状和组合图；整批至少5个视觉家族、3种风格模式和3种页面版式，单一家族不超过40%。词云必须有至少100条清洗文本并与词频柱/主题表成对；所有图形均通过数据门，不能为丰富样式模拟数据。每批须完成Excalidraw、thesis-figure-skill与imagegen三工具路由；研究框架、流程、机制、价值链和路线图默认使用Excalidraw式视觉论证，学术构图由thesis-figure-skill复核或按适配性重绘，位图辅助只用于适配场景，不能伪造企业事实。逐图保留可编辑源文件并完成至少两轮渲染复核；每张图片前必须紧邻一段正文引导，图片内不得嵌入题注，图内所有文字最终不得小于14pt，关键节点、主标签与关键数字使用16pt。正文必须用明确的主语、谓语和宾语说明企业做了什么、数据反映什么以及该事实意味着什么；不得用概括句加冒号引出多个短语，也不得把多个经营动作压缩成引号内的词组链。B03还必须逐项把瓶颈与根因、对应动作、责任和时间预算、同口径KPI及触发回滚条件写成完整句并一一对应，任何无根因方案或有根因无动作均退回修改。
17. 每批正文完成后，从内部施工卡生成只含正式章节、图表和正常来源注的读者稿。先填写去AI味复核表并逐段做语义回归，再对 Markdown 运行 `scripts/validate_reader_facing_report.py`；只有自然表达和证据边界均通过，才用 `$documents:documents` 导出该批 DOCX，并对 DOCX 再运行同一检查。禁止把内部施工卡、修改日志或复核表直接转换为 Word。
18. 每批Word使用 [report-word-format-profile.json](assets/report-word-format-profile.json) 和 `$documents:documents`：A4竖版，上/下2.54cm、左/右3.17cm；正文小四12pt宋体，英文/数字Times New Roman，1.25倍行距、两端对齐、首行2字符；一级14pt黑体加粗并启用“段前分页”，二/三级12pt宋体加粗。除一级标题形成的章节分页外，不得使用手工分页符或分节符制造分页。最终目录使用Word自动域引用全文一级、二级、三级标题。图题放图下、表题放表上，均为居中小五9pt黑体，并使用真实Word `SEQ`域自动编号；图前必须有正文引导，图片内不得嵌入题注，图内所有可见文字在最终Word宽度下不得小于14pt，关键节点、主标签与关键数字为16pt。表格内所有文字水平、垂直居中，单元格段落显式取消首行缩进。量化图表先验证再制作。每个渲染页至少含一张相关且有来源的图片或真正表格；第3—10章叙述性页面至少60%含分析性图片，不能连续两张叙述性页面均无图片。不得用Logo、装饰照、文字截图、色块或段落套表格凑数；参考文献始终为普通编号列表而非表格。
19. 每批复制 [word-visual-manifest-template.json](assets/word-visual-manifest-template.json) 和 [editable-figure-register-template.json](assets/editable-figure-register-template.json)，按真实渲染页和全部图形填写；先运行 `python3 scripts/validate_report_docx_format.py --strict-caption-coverage <batch.docx>`，最终合并稿增加 `--final-report`，再运行Word视觉清单与可编辑母版校验。B02/B03还须对图表地图先运行普通校验、全部图形生成后运行 `--for-production`。用户要求Canva时，可编辑母版生产校验必须包含至少一张经用户批准并提交的 `CANVA_MASTER`；缺少设计ID时不得把该门禁标为通过。读者正文、DOCX格式、图表地图、可编辑母版、页面清单和人工逐页检查均通过后，交付DOCX并等待确认；任何修改均重新刷新域、导出、渲染和校验。
20. 三批 Word 均获确认后，再根据已接受正文编写第2章概要与第1章目录，组装唯一全文 Word/PDF；概要不得先于正文编造结论。合并稿重新执行读者正文检查、证据/因果/地区性/跨章/匿名/原创复核以及全量逐页视觉检查。
