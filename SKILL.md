...
description: 辅导全国高校商业精英挑战赛创新创业竞赛流通业经营模拟赛道的完整备赛流程：项目简报、企业选题与同校查重、地区调研与证据台账、国一经验蒸馏后的分析报告框架和三批分章节写作、权威来源与数据图表验证、专业问卷/访谈附录、中文研究报告Word版式、学术图解润色、自动图题表题与逐页视觉门禁、官方60分证据评分复核、恰好20页PPT初版大纲及frontend-slides制作、证据绑定的10分钟逐页路演稿与真人彩排，以及可选5分钟答辩和最终合规检查。用于用户提到流通业经营模拟赛道、企业经营案例分析报告、流通业计划书、批发/零售/餐饮/物流/供应链/冷链/即时零售案例、选题查重、调研、地区性分析、报告正文、Word排版、图表题注、数据可视化、报告评分、20页PPT、10分钟陈述或答辩时；支持研究生、本科和高职组。必须区分本赛道与创业计划赛道，不得虚构证据或代写成可冒充完全由人类原创的终稿。
---
...
- **报告正文与 Word**：框架确认后必须读 [section-writing-evidence-protocol.md](references/section-writing-evidence-protocol.md)、[reader-facing-report-style.md](references/reader-facing-report-style.md)、[annual-report-empirical-methods.md](references/annual-report-empirical-methods.md)、[word-visual-production.md](references/word-visual-production.md) 和 [excalidraw-imagegen-integration.md](references/excalidraw-imagegen-integration.md)；写B02/B03还必须读 [national-first-b02-b03-writing-visual-patterns.md](references/national-first-b02-b03-writing-visual-patterns.md)。内部施工复制 [report-section-batch-template.md](assets/report-section-batch-template.md)，不得导出；读者正文按批次复制 `report-reader-b01/b02/b03-template.md`。B02/B03先复制 [report-visual-map-template.json](assets/report-visual-map-template.json) 并运行 `scripts/validate_report_visual_map.py`。数据先调用 `$data-analytics:validate-data`，程序分析调用 `$data-analytics:jupyter-notebooks`，统计图调用 `$data-analytics:visualize-data`，每批DOCX调用 `$documents:documents`。每一批正文配图都必须调用并记录外部 `$excalidraw-diagram`、`$thesis-figure-skill` 和系统 `$imagegen` 的路由结果：Excalidraw负责研究框架、价值链、流程、机制、泳道和路线图，thesis-figure-skill负责学术图形的构图审查以及适配的公式/TikZ/高密度机制图重绘，imagegen只辅助适配的无文字位图或视觉素材。某工具不适合实际出图时也要记录 `INVOKED_NOT_APPLICABLE` 及理由，不能为了“调用齐全”生成装饰图。三者均不得替代数据图、精确标签或真实证据。所有结构图须保留可编辑源文件并完成渲染复核；图内不得含题注，图内所有可见文字在最终Word宽度下不得小于14pt，关键节点、主标签与关键数字采用16pt。信息放不下时拆图或移到正文，不得缩字。Kami 1.12.0 只作为版式与视觉语言参考；这些工具都不得写进参赛正文，也不能冒充统计证据或Word生成器。
...
17. 每批正文完成后，从内部施工卡生成只含正式章节、图表和正常来源注的读者稿。对 Markdown 运行 `scripts/validate_reader_facing_report.py`，再用 `$documents:documents` 导出该批 DOCX，并对 DOCX 再运行同一检查；禁止把内部施工卡直接转换为 Word。
...
19. 每批复制 [word-visual-manifest-template.json](assets/word-visual-manifest-template.json)，按真实渲染页填写；先运行 `python3 scripts/validate_report_docx_format.py --strict-caption-coverage <batch.docx>`，最终合并稿增加 `--final-report`，再运行 `python3 scripts/validate_word_visual_manifest.py <manifest.json>`。B02/B03还须对图表地图先运行普通校验、全部图形生成后运行 `--for-production`。读者正文、DOCX格式、图表地图、页面清单和人工逐页检查均通过后，交付DOCX并等待确认；任何修改均重新刷新域、导出、渲染和校验。
