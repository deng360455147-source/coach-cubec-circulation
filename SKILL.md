...
4. 使用 [report-framework-approval-template.md](assets/report-framework-approval-template.md) 输出到三级标题。概要只保留核心问题、主要发现和方案方向；引言只回答“为什么研究、怎么研究”；案例简介只保留理解后文所需的企业业务、地区基础和案例价值。每个三级标题下写 2—3 行结合项目材料的内容概括和证据边界，缺证时保留待处理状态，不得用通用经验补成企业事实。
